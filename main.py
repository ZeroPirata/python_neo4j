...
class App:
    ...
    def relacionamentos(self):
        with self.driver.session(database="neo4j") as session:
            ...
            if escolha == 'vendedor':
                ...
                for row in result:
                    print("Criando relação entre: {p1}, {p2}".format(
                        p1=row['p1'], p2=row['p2']))
            elif escolha == 'pessoa':
    ...
    def querys(self):
        with self.driver.session(database="neo4j") as session:
            ...
            if findQuery == 'produto':
                result = session.execute_read(
                    self._querys_produtos, findQuery)
                for row in result:
                    print("Produto encontrada \nnome: {row[1]} \nquantidade: {row[0]}\nvalor: {row[2]}".format(
                        row=row))
            if findQuery == 'geral':
                ...
                if escolha == 'produto' or escolha == 'vendedor':
                    result = session.execute_read(
                        self._querys, escolha)
                    for row in result:
                        print("\n" + findQuery + " encontrado \nnome: {row[1]} \nemail: {row[0]}\nidade: {row[2]}".format(
                            row=row))
    ...
    def _criar_objeto(banco, tipo):
        if tipo == 'pessoa' or tipo == 'vendedor':
            ...
            resultado = banco.run(query, email_pessoa=email_pessoa,
                                  idade_pessoa=idade_pessoa, nome_pessoa=nome_pessoa)
            return [{"objeto": row['objeto']['email']['idade']['nome']} for row in resultado]
        elif tipo == 'produto':
            query = (
                "CREATE (objeto:"+tipo +
            ...
            resultado = banco.run(query, nome_produto=nome_produto,
                                  quantidade_produto=quantidade_produto, valor_produto=valor_produto)
            return [{"objeto": row['objeto']['valor']['quantidade']['nome']} for row in resultado]
    ...
    def _vendedor_produto(banco):
        ...
        result = banco.run(query, vendedor_nome=vendedor_nome,
                           produto_nome=produto_nome)
        try:
            return [{"p1": row["p1"]["nome"], "p2": row["p2"]["nome"]}
                    for row in result]
